...
Checkout.prototype = {
    ...
    gotoSection: function (section, reloadProgressBlock) {
		...
		if(section == "shipping_method" && jQuery('input[name="shipping_method"]').length > 0){
			jQuery("#s_method_freeshipping_freeshipping").attr("checked", true);
			jQuery("#checkout-step-shipping_method button").trigger("click");
		}
		//~ else{
			//~ if(section == "shipping_method"){
			//~ jQuery("#checkout-shipping-method-load strong").text("We are getting available shipping methods for this order.");
		//~ }
			//~ jQuery("#billing-address-select option[value='']").attr('selected', true);
			//~ jQuery("#billing-buttons-container button").trigger("click");
		//~ }
