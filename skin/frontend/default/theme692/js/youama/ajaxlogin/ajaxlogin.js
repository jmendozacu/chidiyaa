...
(function($) {
    $.fn.youamaAjaxLogin = function(options) { 
        ...
        return start();
		 // window.setTimeout( show_popup, 10000 ); // 5 seconds
        ...
        function openCloseWindowEvents() {
			...
			$('.header .links li:first-child').click(function() {
                 animateShowWindow('register');
                $('div.shadow').addClass('active-form');
                heightWidthForm();
        ...
        function sendEvents() {
            ...
            $('.youama-register-window button.hide-for-joinnow').on('click', function() {
                setDatas('register');
            ...
            // custom code for validate email first

               $('.youama-register-window button.show-for-joinnow').on('click', function() {

                setDatas('register');
                validateDatas('registerfirst');
                if (opts.errors != ''){
                  
                    setError(opts.errors, 'register');
                } else {
                    
                    callAjaxControllerRegistrationFirstStep();
                }
                return false;
            });
            // EOF custom code for validate email first
        ...
        function validateDatas(windowName) {
            ...
            // Login
                ...
                // REGISTER FIRST
            }else if (windowName == 'registerfirst') {
                // There is no email address
                if (opts.email.length < 1) {
                    opts.errors = opts.errors + 'noemail,'
                // It is not email address
        ...
        /**
         * Ajax call for registration first Step.
         */
        function callAjaxControllerRegistrationFirstStep() {
            // If there is no another ajax calling
            //alert('callAjaxControllerRegistrationFirstStep');
            if (opts.stop != true) {
                ...
                ajaxRegistration.done(function(msg) {
                    ...
                    if (msg != 'emailisnew') {
                        ...
                        setTimeout(showLoginPanelExistUser, 2000);
                        
                    // If everything are OK
                    } else {
                        opts.stop = false;
                         jQuery('.youama-window-box .show-for-joinnow').hide();
                        jQuery('.youama-window-box .hide-for-joinnow').show();
                    }
    ...
    function showLoginPanelExistUser(){
        jQuery('#y-to-login').trigger("click");
    }
