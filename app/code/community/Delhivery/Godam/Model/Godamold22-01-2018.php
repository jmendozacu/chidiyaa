	...
	public function getPackageData($orderid, $suborder) {
		...
		foreach($ordered_items as $item){
			...
			if($suborderitem[0] == $item->getItemId())
			{
				...
				$product['prod_desc']= Mage::getModel('catalog/product')->load($item->getProductId())->getShortDescription();
				$product['prod_qty']= intval($item->getQtyOrdered());
				$qty=intval($item->getQtyOrdered());
				$products[]=$product;
				$rowtotal = $item->getRowTotalInclTax();
				$taxamout = $item->getTaxAmount();
		...
		$orderline['express_delivery'] = 'True';
			...
			$invoice['total_price'] = floatval($rowtotalUpdated-$discount); 
