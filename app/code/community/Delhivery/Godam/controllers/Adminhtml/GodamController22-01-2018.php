...
class Delhivery_Godam_Adminhtml_GodamController extends Mage_Adminhtml_Controller_Action {
	...
	public function updateFromGodamAction() {
		...
		foreach ($orders as $gorder) {
				...
				$url =  Mage::getStoreConfig('godam/godam/api_url');
				$suborderid=urlencode($model->suborderid);
				...
				if($result->OrderLine->status):
					...
					if($result->OrderLine->status == 'shipped' && Mage::getStoreConfig('godam/godamorder/invoice_shipment_create')==1)
					{
						Mage::getModel('godam/godam')->createGodamInvoice($model->orderincid);
						Mage::getModel('godam/godam')->completeShipment($model->orderincid,$result->OrderLine->waybill, $result->OrderLine->courier);
