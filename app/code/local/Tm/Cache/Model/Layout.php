...
/**
 * Copy of Mage_Core_Model_Layout methods
 *
 *  _generateBlock is modified
 * Other methods copied because they has no public access
 */
class TM_Cache_Model_Layout
{
    public function initMessages($messagesStorage)
    {
        ...
        foreach ($messagesStorage as $storageName) {
            ...
            if (method_exists($block, 'addStorageType')) {
    ...
    protected function _generateBlock($node, $parent)
    {
        ...
        if ($helper->isBlockSingleton($blockName) && $this->getBlock($blockName)) {
            return $this;
        }

        // tm_cache modification
        $block = new Varien_Object(array(
            'type'           => (string)$node['type'],
            'name_in_layout' => $blockName
        ));

        if (!$helper->isDynamicBlock($block) && !$helper->hasDynamicMethods($block)) {
            ...
            // recursively check all parent blocks.
            if (empty($parentBlock) || $helper->isStaticBlock($parentBlock)) {
                return $this;
            }
        }
        // tm_cache modification
