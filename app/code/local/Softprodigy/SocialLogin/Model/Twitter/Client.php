...
class SoftProdigy_SocialLogin_Model_Twitter_Client
{
    const REDIRECT_URI_ROUTE = 'softprodigy_sociallogin/twitter/connect';
    const REQUEST_TOKEN_URI_ROUTE = 'softprodigy_sociallogin/twitter/request';

    const OAUTH_URI = 'https://api.twitter.com/oauth';
    const OAUTH2_SERVICE_URI = 'https://api.twitter.com/1.1';    

    const XML_PATH_ENABLED = 'softprodigy_sociallogin/twitter/enabled';
    const XML_PATH_CLIENT_ID = 'softprodigy_sociallogin/twitter/api_key';
    const XML_PATH_CLIENT_SECRET = 'softprodigy_sociallogin/twitter/secret';
    ...
    protected function fetchAccessToken()
    {
        if (!($params = Mage::app()->getFrontController()->getRequest()->getParams())
            ...
            throw new Exception(
                Mage::helper('softprodigy_sociallogin')
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401 || $status == 429)) {
                if(isset($decoded_response->error->message)) {
                    ...
                    $message = Mage::helper('softprodigy_sociallogin')
                        ->__('Unspecified OAuth error occurred.');
                }

                throw new SoftProdigy_SocialLogin_TwitterOAuthException($message);
            } else {
                $message = sprintf(
                    Mage::helper('softprodigy_sociallogin')
...
class SoftProdigy_SocialLogin_TwitterOAuthException extends Exception
{}
