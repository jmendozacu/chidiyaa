...
class SoftProdigy_SocialLogin_Model_Yahoo_Client
{
    const REDIRECT_URI_ROUTE = 'softprodigy_sociallogin/yahoo/connect';
    const REQUEST_TOKEN_URI_ROUTE = 'softprodigy_sociallogin/yahoo/request';

    const OAUTH_URI = 'https://api.login.yahoo.com/oauth/v2';
    const OAUTH2_SERVICE_URI = 'http://social.yahooapis.com/v1/user/';

    const XML_PATH_ENABLED = 'softprodigy_sociallogin/yahoo/enabled';
    const XML_PATH_CLIENT_ID = 'softprodigy_sociallogin/yahoo/api_key';
    const XML_PATH_CLIENT_SECRET = 'softprodigy_sociallogin/yahoo/secret';
    ...
    protected function _httpRequest($url, $method = 'GET', $params = array())
    {
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401 || $status == 429)) {
                ...
                throw new SoftProdigy_SocialLogin_YahooOAuthException($message);
            } else {
                $message = sprintf(
                    Mage::helper('softprodigy_sociallogin')
...
class SoftProdigy_SocialLogin_YahooOAuthException extends Exception
{}
