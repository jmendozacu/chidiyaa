 ...
 */
class Softprodigy_MagtrackApi_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function isShowBestsellers(){
        ...
        $show = $settings->getSetting(Softprodigy_MagtrackApi_Model_Settings::_SHOW_BESTSELLERS);
        if($show == 'on'){
            return true;
        }else{
            return false;
    ...
    public function timeBestsellers(){
        ...
        if(is_null($time)) {
            $time = '30d';
            $settings = Mage::getModel('softprodigy_magtrackapi/settings');
            $settings->saveSetting($time, Softprodigy_MagtrackApi_Model_Settings::_TIME_BESTSELLERS);
        }
    ...
    public function getBestsellersDateRangeCode(){
        if($this->_bestseller_date_range_code){
            ...
            return Mage::getSingleton('softprodigy_magtrackapi/settings')
                ->getSetting(Softprodigy_MagtrackApi_Model_Settings::_TIME_BESTSELLERS);
        }
    }
    ...
    public function getBestsellersCollection(){
        ...
        $bestsellers = Mage::getSingleton('softprodigy_magtrackapi/bestsellers')
            ->setStoreId($storeId)
    ...
    public function loadSalesByProduct($product_id){
        ...
        $sales = 0;
        if(isset($product_id)){
            $orders = Mage::getModel('softprodigy_magtrackapi/orders')->getOrderByProduct($product_id,$from);
    ...
    public function getLimit(){
        if($this->_limit_items)
            ...
            return 10;//Mage::getModel('softprodigy_magtrackapi/settings')->getSetting('limit');
    }
    ...
    public function getLastLogin(){
        ...
        if($last == ''){
            $last = Mage::getModel('softprodigy_magtrackapi/settings')->getLastLogin();
    ...
    public function getTimeNewOrders(){
        ...
        if($time == ''){
            $time = Mage::getModel('softprodigy_magtrackapi/settings')->getTimeNewOrders();
    ...
    public function resetTimeNewOrders(){
        ...
        Mage::getModel('softprodigy_magtrackapi/settings')->saveTimeNewOrders($datetime);
    }
    ...
    public function getTimeNewCustomers(){
        ...
        if($time == ''){
            $time = Mage::getModel('softprodigy_magtrackapi/settings')->getTimeNewCustomers();
    ...
    public function resetTimeNewCustomers(){
        ...
        Mage::getModel('softprodigy_magtrackapi/settings')->saveTimeNewCustomers($datetime);
    }
    ...
    public function getLastLoggedOut(){
        return Mage::getModel('softprodigy_magtrackapi/settings')->getLastLogout();
    }
    ...
    public function getNewOrderCollection(){
        $lastTime = Mage::helper('softprodigy_magtrackapi')->getTimeNewOrders();
        $collection = Mage::getResourceModel('sales/order_collection');
        //sort list
        ...
        //filter store
        $storeId = Mage::helper('softprodigy_magtrackapi')->currentStoreId();
        if($storeId != 0){
            $collection->addAttributeToFilter('store_id', $storeId);
        }
        // filted by status
        /*
        $status_text = Mage::getModel('softprodigy_magtrackapi/settings')->getSetting(Softprodigy_MagtrackApi_Model_Settings::_STATUS_ORDERS);
